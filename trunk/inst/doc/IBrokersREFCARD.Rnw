...
{\small IBrokers 0.2-7; TWS API 9.62}
...
and details.
\vskip 5pt
\key{connect}{twsConnect}
\key{disconnect}{twsDisconnect, close}
\key{is a tws connection}{is.twsConnection}
...
\vskip 5pt
\key{place order and process}{placeOrder}
\key{place order (only)}{.placeOrder}
...
\vskip 5pt
\key{subscribe and process}{reqAccountUpdates}
\key{subscribe (only)}{.reqAccountUpdates}
\vskip 5pt
