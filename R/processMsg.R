...
twsCALLBACK <- function(twsCon, eWrapper, timestamp, file, playback=1, ...)
{
  ...
  if(inherits(twsCon, 'twsPlayback')) {
    ...
    while(TRUE) {
      last.time <- sys.time
      sys.time <- as.POSIXct(paste(readBin(con, character(), 2), collapse=' '))
