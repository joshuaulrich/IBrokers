    ...
    if (inherits(con, "sockconn")) {
      ...
      for (n in 1:length(Contract)) {
        if (Contract[[n]]$sectype == "BAG") {
          ...
          for (leg in 1:bag) {
            bag <- c(
              bag, Contract[[n]]$comboleg[[leg]]$conId,
...
reqMktData <- function(conn, Contract, tickGenerics = "100,101,104,106,165,221,225,236",
  ...
  if (inherits(con, "sockconn")) {
    for (n in 1:length(Contract)) {
      ...
      signals <- c(
        ...
        }, # gracefully fix NULL tradingClass
  ...
  CALLBACK(conn,
    eWrapper = eventWrapper, timestamp = timeStamp,
...
", file = file, append = TRUE)
              ...
              eventTickPrice(curMsg, contents, sys.time, file)
            }
...
", file = file, append = TRUE)
              ...
              eventTickSize(curMsg, contents, sys.time, file)
            }
            msg_position <- msg_position + 4
          }
...
", file = file, append = TRUE)
              ...
              eventTickOption(curMsg, contents, sys.time, file)
            }
...
", file = file, append = TRUE)
              ...
              eventTickGeneric(curMsg, contents, sys.time, file)
            }
            msg_position <- msg_position + 4
          }
...
", file = file, append = TRUE)
              ...
              eventTickString(curMsg, contents, sys.time, file)
            }
            msg_position <- msg_position + 4
            if (snapshot == "1") {
              waiting <- FALSE
