...
eWrapper <- function(debug = FALSE, errfile = stderr()) {
  ...
  if (is.null(debug)) {
    ...
    tickPrice <- function(curMsg, msg, timestamp, file, ...) {
      ...
      e_tick_price(
        NULL, msg, timestamp, file, symbols,
        ...
      )
    }
    ...
    updateMktDepthL2 <- function(curMsg, msg, timestamp,
                                     file, ...) {
      symbols <- get.Data("symbols")
      e_update_mkt_depthL2(
        NULL, msg, timestamp, file,
        symbols, ...
      )
    }
    updateNewsBulletin <- function(curMsg, msg, timestamp,
                                       file, ...) {
      cat(
        "newsMsgId: ", msg[2], "newsMsgType: ", msg[3],
    ...
    scannerData <- function(curMsg, reqId, rank, contract,
      ...
      e_scannerData(
        curMsg, reqId, rank, contract, distance,
  ...
  eW <- list(
    .Data = .Data, get.Data = get.Data, assign.Data = assign.Data,
