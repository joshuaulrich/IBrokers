eWrapper <-
function (debug = FALSE, errfile = stderr()) 
{
    .Data <- new.env()
    ...
    if (is.null(debug)) {
        ...
        commissionReport <- function(curMsg, msg, timestamp, 
            file, ...) {
            e_commissionReport(curMsg, msg, file, ...)
        }
        marketDataType <- positionData <- positionEnd <- accountSummary <- accountSummaryEnd <- verifyMessageAPI <- verifyCompleted <- displayGroupList <- displayGroupUpdated <- function(curMsg, 
            msg, timestamp, file, ...) {
            c(curMsg, msg)
        }
    }
    ...
    eW <- list(.Data = .Data, get.Data = get.Data, assign.Data = assign.Data, 
        ...
        marketDataType = marketDataType, commissionReport = commissionReport, 
        positionData = positionData, positionEnd = positionEnd, 
        accountSummary = accountSummary, accountSummaryEnd = accountSummaryEnd, 
        verifyMessageAPI = verifyMessageAPI, verifyCompleted = verifyCompleted, 
        displayGroupList = displayGroupList, displayGroupUpdated = displayGroupUpdated)
    class(eW) <- "eWrapper"
    invisible(eW)
}
