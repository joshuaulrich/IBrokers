...
environment from a user standpoint involves a single
...
{\tt R} is a statistical programming language based on the
{\tt S} dialect created by Chambers et al at Bell Laboratories.
...
is called, and it returns a connection object.  This
connection can be specified to be 
...
for the purposes of flexibility and robustness.
